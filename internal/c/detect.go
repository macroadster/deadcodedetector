...
func Detect(root string, files []walk.File, entries []string) ([]finding.Finding, error) {
	...
	libProject := len(dylibs) > 0 || looksLikeSharedLibProject(root) || hasDllexport(byAbs)
	reportPublic := hasMain || len(dynSyms) > 0 || len(entrySet) > 0 && !libProject
	// Library-only trees keep non-static symbols as API unless a consumer
	// (main or dlsym) is in the scan.
	...
	for abs, u := range byAbs {
		...
		fs = append(fs, unusedMacros(u, byAbs, liveFiles, reportPublic)...)
		fs = append(fs, unusedDecls(u, globalUses, reportPublic)...)
	}
...
func unusedIncludes(u *unit, byAbs map[string]*unit) []finding.Finding {
	...
	for _, inc := range u.Includes {
		...
		// Barrel / aggregator headers re-export nested includes — prefer FN.
		if len(h.Includes) > 0 {
			continue
		}
		names := 0
...
func unusedMacros(u *unit, byAbs map[string]*unit, live map[string]bool, reportPublic bool) []finding.Finding {
	// Count uses of each macro name across live files (and this file).
	uses := map[string]int{}
	for abs, other := range byAbs {
		if live != nil && !live[abs] && len(live) > 0 {
			continue
		}
	...
	for _, m := range u.Macros {
		...
		// Library public headers: keep unused macros as API when we have
		// no consumer in-tree.
		if u.IsHeader && !reportPublic {
			continue
		}
		fs = append(fs, finding.Finding{
			Language: finding.C,
...
func unusedDecls(u *unit, global map[string]int, reportPublic bool) []finding.Finding {
	local := localUseCount(u)
	var fs []finding.Finding
	seen := map[string]bool{}
	for _, d := range u.Decls {
		...
		if isTestFile(u.File.Rel) && looksLikeTestName(d.Name) {
			continue
		}
		if !d.Static && !reportPublic && !d.Exported {
			// Non-static symbols in a library-only tree are the public API.
			continue
		}
