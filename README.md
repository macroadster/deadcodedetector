...
| **C** | Unused `static` functions and file-scope variables, unused internal (non-`include/`) functions when a `main` or `dlsym` consumer exists, unused `#define` macros in `.c` / private headers, unused local `#include`s, and unused stray files. `test()` / `libNNNN.c` harnesses are entries. Public `include/` API, `config-`/`setup-` headers, and `##` token-paste macros are kept. Dynamic-library exports from `.so` / `.dylib` / `.dll` that are never referenced via a call or `dlsym("name")` are reported. |
...
**C.** Tokenizes C (including preprocessor directives), extracts `#include`, `#define`, file-scope functions / variables, and identifier uses. `#if 0` blocks are skipped. Include-guard macros (`#ifndef FOO_H` / `#define FOO_H`) are never reported. `dlsym("name")` / `GetProcAddress(..., "name")` (including `TEXT("name")`) and `dlopen("lib.so")` count as uses of that export / library. `__attribute__((constructor|destructor|used))`, `DllMain` / `main`, and harness `test()` (`lib1500.c`, `unit1300.c`) are entry points. A tree with `include/` + `lib/` (or `add_library(... SHARED)`) keeps symbols declared in public headers even if a CLI `main` exists. Macros constructed with `##`, names in `config-`/`setup-` headers, and `true`/`false`/`NULL` are kept. `.c` files under `lib/` / `tests/` / `projects/` are build-selected TUs and are not reported as unused files. Compiled `.so` / `.dylib` / `.dll` files are opened (ELF / Mach-O / PE) and unused exported symbols that nothing `dlsym`s or calls are reported.
...
- C is tokenized, not compiled: `#if` conditions other than `#if 0` are assumed live, computed `dlsym(h, name)` keeps every export (prefer FN), and C++ mangled names in a `.so` are ignored. Unused `#include`s under `lib/` / `include/` / `tests/` are not reported. Public `include/` macros and `##` paste results are kept (prefer FN), so a truly unused public alias such as `CURL_STRICTER` is not reported.
